function run() {
	...
	let scheme = selection[0] as FrameNode
	scheme.children.forEach(
		function(eachSwatch) {
			...
			let ellipse = eachSwatch.children[0] as EllipseNode
			let text = eachSwatch.children[1] as TextNode
			...
			let style = styles.find(eachStyle =>
				eachStyle.name == styleName || eachStyle.name == styleName.replace(/\s/g, "")
			)
